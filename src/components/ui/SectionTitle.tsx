...
export function SectionTitle({ over, title, align = 'center' }: SectionTitleProps) {
  ...
  return (
    <div className={`${textAlign} mb-10`}>
      ...
      <h2 className="font-display text-[clamp(28px,4vw,44px)] text-brand-text dark:text-dark-text font-bold mt-1 mb-3.5 leading-tight">
        {title}
      </h2>
      <div
        className={`w-[70px] h-[3px] rounded-full bg-gradient-to-r from-primary to-accent ${ruleMargin}`}
      />
    </div>
  );
}
